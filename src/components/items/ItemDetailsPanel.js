...
export default function ItemDetailsPanel({ item, onRemove }) {
  if (!item) {
    return (
  ...
  return (
    <div className="w-80 bg-white rounded-xl shadow-sm p-5 flex flex-col">
      ...
      <span className={`inline-block w-fit px-3 py-1 rounded-full text-xs ${status.color}`}>
        {status.label}
      </span>
      ...
      <div className="mt-auto pt-6">
        <button
          ...
          className="w-full border border-red-300 text-red-500 rounded-lg py-2 hover:bg-red-50"
        >
          Remove
        </button>
      </div>
